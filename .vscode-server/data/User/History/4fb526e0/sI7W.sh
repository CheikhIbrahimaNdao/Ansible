...
ansible-playbook $ansible_playbook_path --extra-vars "{{ item.name }} db_user={{ item.user }} db_password={{ item.password }} mysql_root_password=$mysql_root_password" --ask-become-pass
{% endfor %}
